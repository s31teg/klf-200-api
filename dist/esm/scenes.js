...
import { GW_GET_SCENE_INFORMATION_NTF } from "./KLF200-API/GW_GET_SCENE_INFORMATION_NTF.js";
...
export class Scene extends Component {
    ...
    async runAsync(Velocity = 0, PriorityLevel = 3, CommandOriginator = 1) {
        ...
        const confirmationFrame = await this.Connection.sendFrameAsync(new GW_ACTIVATE_SCENE_REQ(this.SceneID, PriorityLevel, CommandOriginator, Velocity));
    ...
    async stopAsync(PriorityLevel = 3, CommandOriginator = 1) {
        ...
        const confirmationFrame = await this.Connection.sendFrameAsync(new GW_STOP_SCENE_REQ(this.SceneID, PriorityLevel, CommandOriginator));
        if (confirmationFrame.Status === ActivateSceneStatus.OK) {
    ...
    async refreshAsync() {
        ...
        try {
            ...
            }, [GatewayCommand.GW_GET_SCENE_INFORMATION_NTF]), false);
...
export class Scenes {
    Connection;
    _onChangedScenes = new TypedEvent();
    _onRemovedScenes = new TypedEvent();
    _onAddedScenes = new TypedEvent();
    /**
    ...
    async refreshScenesAsync() {
        ...
        try {
            ...
            }, [GatewayCommand.GW_GET_SCENE_LIST_NTF]), false);
