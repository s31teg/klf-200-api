...
class Product extends PropertyChangedEvent_js_1.Component {
    ...
    async refreshLimitationAsync(limitationType, parameterActive = GW_COMMAND_js_1.ParameterActive.MP) {
        try {
            ...
            const dispose = this.setupWaitForLimitationFinished(frameToSend.SessionID, limitationType, parameterActive, resolve, reject);
            try {
                let disposeSessionFinishedNtf;
                try {
                    const waitForSessionFinishedNtfPromise = new Promise((res, rej) => {
                        try {
                            disposeSessionFinishedNtf = this.Connection.on((frame) => {
...
class Products {
    ...
    async initializeProductsAsync() {
        ...
        try {
            ...
            dispose = this.Connection.on((frame) => {
                try {
                    debug(`Calling handler for GW_GET_ALL_NODES_INFORMATION_NTF, GW_GET_ALL_NODES_INFORMATION_FINISHED_NTF in Products.initializeProductsAsync.`);
                    if (frame instanceof GW_GET_ALL_NODES_INFORMATION_NTF_js_1.GW_GET_ALL_NODES_INFORMATION_NTF) {
                        ...
                        this.Connection.on(async (frame) => {
                            debug(`Calling handler for GW_CS_SYTEM_TABLE_UPDATE_NTF in Products.initializeProductsAsync.`);
                            await this.onNotificationHandler(frame);
                ...
                common_js_1.GatewayCommand.GW_GET_ALL_NODES_INFORMATION_NTF,
                common_js_1.GatewayCommand.GW_GET_ALL_NODES_INFORMATION_FINISHED_NTF,
            ]);
            ...
            if (getAllNodesInformation.Status !== common_js_1.GW_COMMON_STATUS.SUCCESS) {
                ...
                if (getAllNodesInformation.Status !==
                    common_js_1.GW_COMMON_STATUS.ERROR /* No nodes available -> not a real error */) {
    ...
    async notifiyRemovedProduct(nodeId) {
