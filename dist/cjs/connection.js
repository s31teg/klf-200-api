...
class Connection {
    ...
    async logoutAsync(timeout = 10) {
        ...
        try {
            ...
            if (this.sckt) {
                ...
                await (0, promise_timeout_1.timeout)(new Promise((resolve, reject) => {
                    try {
                        // Close socket
    ...
    async sendFrameAsync(frame, timeout = 10) {
        try {
            debug(`sendFrameAsync called with frame: ${(0, UtilityFunctions_js_1.stringifyFrame)(frame)}, timeout: ${timeout}.`);
            ...
            try {
                ...
                try {
                    ...
                    const cfmHandler = __addDisposableResource(env_2, this.klfProtocol.on((notificationFrame) => {
                        try {
                            debug(`sendFrameAsync frame received: ${(0, UtilityFunctions_js_1.stringifyFrame)(notificationFrame)}.`);
                            if (notificationFrame instanceof GW_ERROR_NTF_js_1.GW_ERROR_NTF) {
                                debug(`sendFrameAsync GW_ERROR_NTF received: ${(0, UtilityFunctions_js_1.stringifyFrame)(notificationFrame)}.`);
                                ...
                                debug(`sendFrameAsync expected frame received: ${(0, UtilityFunctions_js_1.stringifyFrame)(notificationFrame)}.`);
                ...
                debug(`sendFrameAsync error occurred: ${typeof error === "string" ? error : JSON.stringify(error)} with frame sent: ${(0, UtilityFunctions_js_1.stringifyFrame)(frame)}.`);
                reject(error);
                return Promise.reject(error);
            }
        }
        catch (error) {
            debug(`sendFrameAsync error occurred (outer): ${typeof error === "string" ? error : JSON.stringify(error)} with frame sent: ${(0, UtilityFunctions_js_1.stringifyFrame)(frame)}.`);
            return Promise.reject(error);
        }
    }
    /**
    ...
    async notifyFrameSent(frame) {
        debug(`notifyFrameSent called with frame: ${(0, UtilityFunctions_js_1.stringifyFrame)(frame)}.`);
